...
const DATA_DIR =
  process.env.ACTUAL_API_DATA_DIR || "/var/cache/actual-budget-api";
...
async function ensureBudgetDownloaded(
  apiClient: ActualApiClient,
  budgetId: UUID,
  encryptPassword?: string,
) {
  await apiClient.downloadBudget(
    budgetId,
    encryptPassword ? { password: encryptPassword } : {},
  );
}
...
app.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { password, encryptPassword, ...transaction } =
    ...
    try {
      ...
      transaction.cleared =
        transaction.cleared == undefined ? false : transaction.cleared;
      transaction.amount = transaction.amount * 100;

      const result = await apiClient.addTransactions(transaction.account, [
        transaction,
      ]);
...
// Get Transactions
app.post(
  "/budget/:budgetId/accounts/:accountId/transactions",
  validateAuth,
  asyncHandler(async (req: Request, res: Response) => {
    const budgetId = req.params.budgetId as UUID;
    const accountId = req.params.accountId as UUID;
    const { startDate, endDate, password, encryptPassword } =
      req.body as TransactionSearch;

    const apiClient = await initApiClient(password);
    await ensureBudgetDownloaded(apiClient, budgetId, encryptPassword);

    try {
      let transactions = await apiClient.getTransactions(
        accountId,
        startDate,
        endDate,
      );
